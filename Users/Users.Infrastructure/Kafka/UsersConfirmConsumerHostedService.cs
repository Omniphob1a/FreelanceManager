...
{
	public sealed class UsersConfirmConsumerHostedService : BackgroundService
	{
		private readonly ILogger<UsersConfirmConsumerHostedService> _logger;
		...
		private const string Topic = "user-confirm-requests";
		private static readonly TimeSpan ConsumeTimeout = TimeSpan.FromMilliseconds(500);

		public UsersConfirmConsumerHostedService(
			ILogger<UsersConfirmConsumerHostedService> logger,
			IServiceProvider sp,
		...
		public override Task StartAsync(CancellationToken cancellationToken)
		{
			_effectiveGroup = string.IsNullOrWhiteSpace(_settings.GroupId)
				? $"users-confirm-{Topic}-{Guid.NewGuid():n}".Substring(0, 20)
				: _settings.GroupId;

			_logger.LogInformation("UsersConfirm consumer group = {Group} (consumer will be created in ExecuteAsync)", _effectiveGroup);
		...
		protected override async Task ExecuteAsync(CancellationToken stoppingToken)
		{
			...
			while (!stoppingToken.IsCancellationRequested)
			{
				try
				{
					...
					_logger.LogInformation("UsersConfirm consumer created; group = {Group}", _effectiveGroup);
					break;
				}
				catch (Exception ex)
				{
			...
			try
			{
				try
				{
					...
					_logger.LogInformation("UsersConfirm consumer subscribed to {Topic}", Topic);
				}
				catch (Exception ex)
				{
				...
				while (!stoppingToken.IsCancellationRequested)
				{
					...
					try
					{
						...
						{
							_logger.LogError(ex, "Failed to save incoming confirm request; will wait and continue");
					...
					{
						_logger.LogError(ex, "UsersConfirm consumer loop error");
		...
		{
			var effectiveGroup = _effectiveGroup ?? (_settings.GroupId ?? $"users-confirm-{Topic}-{Guid.NewGuid():n}".Substring(0, 20));
