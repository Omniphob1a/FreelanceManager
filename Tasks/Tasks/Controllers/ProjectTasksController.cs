...
namespace Tasks.Api.Controllers
{
	...
	public class ProjectTasksController : ControllerBase
	{
		...
		[HttpGet]
		[ProducesResponseType(typeof(PaginatedResult<ProjectTaskDto>), StatusCodes.Status200OK)]
		public async Task<IActionResult> GetTasks([FromQuery] ProjectTaskFilter filter, CancellationToken ct)
		{
			try
			{
				filter ??= new ProjectTaskFilter();
				// default filter values: if necessary restrict by reporter/assignee/etc by UI rules
				if (filter.Page < 1) filter.Page = 1;
				if (filter.PageSize <= 0) filter.PageSize = 20;
				if (filter.PageSize > 100) filter.PageSize = 100;

				var query = new GetProjectTasksByFilterQuery(filter);
				var result = await _mediator.Send(query, ct);

				if (result.IsFailed)
				{
					_logger.LogWarning("GetTasks failed: {Errors}", result.Errors.Select(e => e.Message));
					return BadRequest(result.Errors.Select(e => e.Message));
				}

				_logger.LogInformation("Tasks fetched. Total: {Total}", result.Value?.Pagination.TotalItems ?? 0);
				return Ok(result.Value);
			}
			catch (Exception ex)
			{
				_logger.LogError(ex, "Unhandled exception in GetTasks");
		...
		public async Task<IActionResult> GetTaskById(Guid taskId, CancellationToken ct)
		{
			...
			var result = await _mediator.Send(new GetProjectTaskByIdQuery(taskId), ct);
		...
		public async Task<IActionResult> AddTimeEntry(Guid taskId, [FromBody] AddTimeEntryRequest request, CancellationToken ct)
		{
			...
			var command = request.Adapt<AddTimeEntryCommand>() with
			{
				TaskId = taskId,
		...
		[ProducesResponseType(StatusCodes.Status404NotFound)]
		public async Task<IActionResult> GetTimeEntries(Guid taskId, CancellationToken ct)
		{
			_logger.LogInformation("Fetching time entries for task {TaskId}", taskId);

			if (taskId == Guid.Empty)
				return BadRequest(new { errors = new[] { "TaskId is required." } });

			var result = await _mediator.Send(new GetTimeEntriesByTaskQuery(taskId), ct);

			if (result.IsFailed)
			{
				var errors = result.Errors.Select(e => e.Message).ToList();
				_logger.LogWarning("Failed to fetch time entries for task {TaskId}. Errors: {Errors}", taskId, errors);
				return NotFound(new { errors });
			}
