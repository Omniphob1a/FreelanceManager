...
{
	public class LogTimeCommandHandler : IRequestHandler<LogTimeCommand, Result<Unit>>
	{
		...
		public async Task<Result<Unit>> Handle(LogTimeCommand request, CancellationToken cancellationToken)
		{
			...
			try
			{
				var period = TimeRange.Create(request.StartedAt, request.EndedAt);
				Money? rate = null;
